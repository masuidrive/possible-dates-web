...
export default class CalendarLoaderComponent extends React.Component {
      ...
      .then(({ result }) => {
        this.setState({
          ...
          calendars: _.fromPairs(result.items, cal =>
            [cal.id, { calendar: cal, eventsOfDay: {} }]
          ),
  ...
  loadActiveCalendarEvents(request) {
    ...
    if(isLoaded) {
      ...
      this.state.activeCalendars.forEach(calId => this.loadCalendarEvents(calId, request));
    }
  }
  ...
  loadCalendarEvents(calendarId, request) {
    ...
    ApiCalendar.gapi.client.calendar.events
      ...
      .then(({ result }) => {
        ...
        utils._enumMomentDays(request.startAt, request.endAt, date => {
          const dateStr = date.format("YYYYMMDD");
          if(events[dateStr] === undefined) { events[dateStr] = [] }
          
          const startOfDayAt = moment(date).startOf("day");
          const endOfDayAt = moment(startOfDayAt).endOf("day");
          events[dateStr] = resultEvents.filter(
            e => utils._isOverrappedMoment(e.start, e.end, startOfDayAt, endOfDayAt)
          );
          // requestが全部読み込み終わったらonLoadEventsを呼び出す
          this._raiseOnLoadEvents(request, this.state.activeCalendars.map(calId => calendars[calId]));
        });
  ...
  _raiseOnLoadEvents(request, calendars) {
    if(calendars === undefined) {
      calendars = this.state.activeCalendars.map(calId => this.state.calendars[calId]);
    }
    var isLoaded = true;
    var events = [];
    utils._enumMomentDays(request.startAt, request.endAt, date => {
